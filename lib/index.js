module.exports = function (share) {
  ...
  task('info', function () {
    ...
    fs.readFile('./package.json', function (err, data) {
      ...
      // quick hack to allow us to specify different target servers for live/dev etc. deployments
      // rather than just different locations on the same server as was case in original version
      //
      // To use set the env key in share.env then use something like the following in your package.json
      // 
      // Assume we can set either 'live' or 'dev' in our share.env then a package.json 
      // might look like this:
      //
      // "launchconf" : {
      // 	"dev" : {
      // 		"remote" : "development.server.com",
      // 		"remotepath" : "/var/web/"
      // 		},
      // 	"live": {
      // 		"remote" : "live.otherserver.com",
      // 		"remotepath" : "/var/web/"
      // 		},
      //   "remote" : "normaluse.remote.com",
      //   "remotepath" : "/var/web/"
      // }

      if (share.env) {
	      if (pkg.launchconf[share.env]) {
		      share.info.remote = pkg.launchconf[share.env].remote;
		      share.info.remotepath = pkg.launchconf[share.env].remotepath;
	      }
      }
